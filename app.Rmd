...
<small>Simulação de Análise de Sobrevivência</small><br>
<small>Rodrigo Reginato Kakuta Kato</small>
</div>
...
surv_data_and_plot <- reactive({
  ...
  if ("A" %in% unique(dados$Group) && "B" %in% unique(dados$Group)) {
    temposA <- dados |> filter(Group == "A") |> pull(Time)
  ...
  for (grupo in c("A", "B")) {
    for (i in seq_along(interval_labels)) {
      ...
      if (percent > 0) {
        idx <- which(dados$Group == grupo & dados$Interval == interval)
        n_idx <- length(idx)
  ...
  g$plot <- g$plot + 
    scale_color_manual(
      values = c("#E41A1C", "#377EB8")[1:length(legend_labs)], 
      labels = legend_labs, 
      name = "Grupo"
      ) +
    scale_fill_manual(
...
output$Statistics_simple <- renderPrint({
  ...
  cat("Logrank p-value: ", survdiff(Surv(Time, Event) ~ Group, 
                                    data = df)$pvalue) 
  cat("\n-------------------------------------------------------------------\n")
...
output$download_script <- downloadHandler(
  ...
  content = function(file) {
    censura_percentual_A <- sapply(seq_along(interval_labels), 
                                   function(i) input[[paste0("censA_", i)]])
    censura_percentual_B <- sapply(seq_along(interval_labels), 
                                   function(i) input[[paste0("censB_", i)]])
    remocao_percentual_A <- sapply(seq_along(interval_labels), 
                                   function(i) input[[paste0("remA_", i)]])
    remocao_percentual_B <- sapply(seq_along(interval_labels), 
...
output$download_grafico <- downloadHandler(
  filename = function() {
    ...
    nome_grafico <- sub("\\.jpeg$|\\.jpg$", "", 
                        nome_grafico, ignore.case = TRUE)
